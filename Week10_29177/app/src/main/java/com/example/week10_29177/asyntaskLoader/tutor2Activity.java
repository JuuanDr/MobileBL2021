package com.example.week10_29177.asyntaskLoader;
...
public class tutor2Activity extends AppCompatActivity  implements LoaderManager.LoaderCallbacks<Void> {
    ...
    static class ContohLoader extends AsyncTaskLoader<Void> {
        static WeakReference<tutor2Activity> mActivity;
        int mCounter = 0;

        public ContohLoader(@NonNull Context context, int n, tutor2Activity main) {
            super(context);
            mCounter = n;
            mActivity = new WeakReference<tutor2Activity>(main);
        }
        @Nullable
        @Override
